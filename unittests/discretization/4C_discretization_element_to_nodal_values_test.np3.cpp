...
namespace
{
  ...
  TEST_F(ElementToNodalValuesTest, ElementToNodalUniformScalarMaterialValues)
  {
    ...
    if (Core::Communication::my_mpi_rank(comm_) == 0)
    {
      ...
      EXPECT_NEAR(nodal_values[17], 20.0, 1e-12);
    }
    else if (Core::Communication::my_mpi_rank(comm_) == 2)
    {
      EXPECT_NEAR(nodal_values[0], 20.0, 1e-12);
      EXPECT_NEAR(nodal_values[8], 20.0, 1e-12);
    }
  }
  ...
  TEST_F(ElementToNodalValuesTest, ElementToNodalHeterogeneousScalarValue)
  {
    ...
    if (Core::Communication::my_mpi_rank(comm_) == 0)
    {
      ...
      EXPECT_NEAR(nodal_values[6], 50.5, 1e-12);
      // nodes inside the second half of domain hold value 100.0
      EXPECT_NEAR(nodal_values[12], 100.0, 1e-12);
    }
    else if (Core::Communication::my_mpi_rank(comm_) == 2)
    {
      // nodes inside the first half of domain hold value 1.0
      EXPECT_NEAR(nodal_values[0], 1.0, 1e-12);
      // nodes on the boundary hold the average so 50.5
      EXPECT_NEAR(nodal_values[3], 50.5, 1e-12);
      // nodes inside the second half of domain hold value 100.0
      EXPECT_NEAR(nodal_values[6], 100.0, 1e-12);
    }
  }
  ...
  TEST_F(ElementToNodalValuesTest, ElementToNodalHeterogeneousTensorValue)
  {
    ...
    if (Core::Communication::my_mpi_rank(comm_) == 0)
    {
      ...
      // Second multivector column
      {
        ...
        // nodes on the boundary hold the average so 50.5
        EXPECT_NEAR(nodal_values[6 + nodal_value_vector.MyLength()], 50.5, 1e-12);
        // nodes inside the second half of domain hold value 1.0
        EXPECT_NEAR(nodal_values[12 + nodal_value_vector.MyLength()], 1.0, 1e-12);
      }
      ...
      // Third multivector column
      {
        ...
        // nodes on the boundary hold the average so 50.5
        EXPECT_NEAR(nodal_values[6 + 2 * nodal_value_vector.MyLength()], 50.5, 1e-12);
        // nodes inside the second half of domain hold value 100.0
        EXPECT_NEAR(nodal_values[12 + 2 * nodal_value_vector.MyLength()], 100.0, 1e-12);
      }
    }
    else if (Core::Communication::my_mpi_rank(comm_) == 2)
    {
      ...
      // Second multivector column
      {
        ...
        EXPECT_NEAR(nodal_values[0 + nodal_value_vector.MyLength()], 100.0, 1e-12);
        // nodes on the boundary hold the average so 50.5
        EXPECT_NEAR(nodal_values[3 + nodal_value_vector.MyLength()], 50.5, 1e-12);
        // nodes inside the second half of domain hold value 1.0
        EXPECT_NEAR(nodal_values[6 + nodal_value_vector.MyLength()], 1.0, 1e-12);
      }
      ...
      // Third multivector column
      {
        ...
        EXPECT_NEAR(nodal_values[0 + 2 * nodal_value_vector.MyLength()], 1.0, 1e-12);
        // nodes on the boundary hold the average so 50.5
        EXPECT_NEAR(nodal_values[3 + 2 * nodal_value_vector.MyLength()], 50.5, 1e-12);
        // nodes inside the second half of domain hold value 100.0
        EXPECT_NEAR(nodal_values[6 + 2 * nodal_value_vector.MyLength()], 100.0, 1e-12);
      }
    }
  }
}  // namespace
