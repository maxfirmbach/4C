...
namespace
{
  ...
  class BuildParallelThermoDiscretizationTest : public testing::Test
  {
   public:
    BuildParallelThermoDiscretizationTest()
    {
  ...
  TEST_F(BuildParallelThermoDiscretizationTest, SetupThermoDiscretization)
  {
    ...
    if (Core::Communication::my_mpi_rank(comm_) == 0 and
        Core::Communication::my_mpi_rank(comm_) == 2)
    {
      auto thermo_material = std::dynamic_pointer_cast<Mat::Fourier>(
          test_discretization_->l_row_element(0)->material());
