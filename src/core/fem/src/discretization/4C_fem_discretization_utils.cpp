...
/*----------------------------------------------------------------------------*
 *----------------------------------------------------------------------------*/
Core::LinAlg::MultiVector<double> Core::FE::Utils::element_to_nodal_values(
  ...
  for (const auto& node : discretization.my_row_node_range())
  {
    ...
    for (int col = 0; col < nodal_material_vector.NumVectors(); col++)
    {
      ...
      for (int k = 0; k < num_elements; k++)
      {
        ...
        nodal_value = 0.5 * (nodal_value + element_vector[local_element_id]);
      }
      nodal_material_vector.ReplaceGlobalValue(node->id(), col, nodal_value);
    }
  }
